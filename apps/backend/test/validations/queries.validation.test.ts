...
describe('Query validations', () => {
  ...
  describe('articles (pagination)', () => {
    it('throws error when limit is not a positive integer', async () => {
      await expect(
        resolvers.Query.articles(null, { limit: 0, offset: 0 })
      ).rejects.toThrow('`limit` must be a positive integer');

      await expect(
        resolvers.Query.articles(null, { limit: -5, offset: 0 })
      ).rejects.toThrow('`limit` must be a positive integer');

      await expect(
        resolvers.Query.articles(null, { limit: 1.5, offset: 0 })
      ).rejects.toThrow('`limit` must be a positive integer');
    });

    it('throws error when offset is negative', async () => {
      await expect(
        resolvers.Query.articles(null, { limit: 10, offset: -1 })
      ).rejects.toThrow('`offset` must be a positive integer');
    });

    it('throws error when offset is not an integer', async () => {
      await expect(
        resolvers.Query.articles(null, { limit: 10, offset: 1.5 })
      ).rejects.toThrow('`offset` must be a positive integer');
    });
  });
  ...
  describe('article (single)', () => {
    it('throws error when id is empty or whitespace', async () => {
      await expect(
        resolvers.Query.article(null, { id: '' })
